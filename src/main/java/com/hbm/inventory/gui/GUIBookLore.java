...
import static net.minecraft.init.SoundEvents.UI_BUTTON_CLICK;
...
public class GUIBookLore extends GuiScreen {
    ...
    @Override
    public void initGui() {
        if(key == null || key.isEmpty()) this.mc.player.closeScreen();
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
        ...
        drawModalRectWithCustomSizedTexture(guiLeft, guiTop, 0, 0, sizeX, sizeY, 512, 512);
        GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
        drawModalRectWithCustomSizedTexture(guiLeft + 7, guiTop + 7, 0, 182, 258, 165, 512, 512);

        final boolean overY = j >= guiTop + 155 && j < guiTop + 165;
        if(page > 0) {
            if(overY && i >= guiLeft + 24 && i <= guiLeft + 42)
                drawModalRectWithCustomSizedTexture(guiLeft + 24, guiTop + 155, 295, 13, 18, 10, 512, 512);
            else
                drawModalRectWithCustomSizedTexture(guiLeft + 24, guiTop + 155, 272, 13, 18, 10, 512, 512);
        }

        if(page < maxPage) {
            if(overY && i >= guiLeft + 230 && i <= guiLeft + 248)
                drawModalRectWithCustomSizedTexture(guiLeft + 230, guiTop + 155, 295, 0, 18, 10, 512, 512);
            else
                drawModalRectWithCustomSizedTexture(guiLeft + 230, guiTop + 155, 272, 0, 18, 10, 512, 512);
        }
    }
        ...
        for(int i = 0; i < 2; i++) {
            ...
            if(defacto < tag.getInteger("p")) {
                ...
                for(int l = 0; l < lines.size(); l++) {
                    this.fontRenderer.drawString(lines.get(l),
    ...
    protected void mouseClicked(int i, int j, int k) {
        ...
        if(page > 0 && i >= guiLeft + 24 && i <= guiLeft + 42) {
            page--;
            mc.getSoundHandler().playSound(PositionedSoundRecord.getMasterRecord(UI_BUTTON_CLICK, 1.0F));
        }
